...
int string_hasextension(const char*file,const char*ext)
{
 size_t len=strlen(file),elen=strlen(ext);
 if((elen==0)&&(len>4)&&(file[len-1]!='.')&&(file[len-2]!='.')&&(file[len-3]!='.')&&(file[len-4]!='.')&&(file[len-5]!='.'))
  return 1;
 else 
  return (len>elen+1)&&(strcmp(file+len-elen,ext)==0)&&(file[len-elen-1]=='.');
}
